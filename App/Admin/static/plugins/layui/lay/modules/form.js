      ...
      this.config = {
        verify: {
          ...
          height: function(value) {
            if (!value || isNaN(value) || value < 1 || value > 250)
              return '身高只能输入1~250之间的整数'
          },
          weight: function(value) {
            if (!value || isNaN(value) || value < 1 || value > 500)
              return '体重只能输入1~500之间的整数'
          }
        }
      };
    };
                ...
                var reElem = $(['<div class="' + (isMultiple ? '' : 'layui-unselect ') + CLASS + (disabled ? ' layui-select-disabled' : '') + '">', '<div class="' + TITLE + '"><input type="text" class="layui-input" placeholder="' + placeholder + '"><div class="layui-input multiSelect" >' + function() {
                  ...
                  if (value != null && value != undefined && value.length != 0) {
                    for (var aList = 0; aList < value.length; aList++) {
                      if (value[aList]) {
                    	// 不应该直接把value当成下标来用
                    	for(var actualIndex = 0; actualIndex < select.options.length; actualIndex++){
                    		if(select.options[actualIndex].value == value[aList]){
                    			aLists.push("<a href='javascript:;'><span>" + select.options[actualIndex].text + "</span><i></i></a>")
                    			break;
                    		}
                    	}
                      }
